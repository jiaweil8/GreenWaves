...
public class RCCP_AeroDynamics : RCCP_Component {
    ...
    private void FixedUpdate() {
        ...
        if (ignoreRigidbodyDragOnAccelerate)
            CarController.Rigid.drag = defaultDrag * (1f - CarController.throttleInput_V);

        // Local forward speed (z-axis).
        float linearSpeed = transform.InverseTransformDirection(CarController.Rigid.velocity).z;
        ...
        CarController.Rigid.AddForceAtPosition(-CarController.Rigid.velocity.normalized * dragForce, COM.position, ForceMode.Force);
